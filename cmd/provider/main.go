...
func main() {
	var (
		app              = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for XenOrchestra").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-jet-xenorchestra"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID:           "crossplane-leader-election-provider-jet-xenorchestra",
	...
	kingpin.FatalIfError(apis.AddToScheme(mgr.GetScheme()), "Cannot add XenOrchestra APIs to scheme")
	...
	kingpin.FatalIfError(controller.Setup(mgr, o), "Cannot setup XenOrchestra controllers")
